...
echo $img_pathComplete='https://anuncio360.com/360/images/foto_perfil/'.$token_R.'.jpg';
  ...
  echo   $query_produto = "INSERT INTO usuario (usuario,senha,data_cadastro,token,tipo_negocio,endereco,telefone,latitude,longitude,email,Foto_perfil) VALUES (:usuario,:senha,:data_cadastro,:token,:tipo_negocio,:endereco,:telefone,:latitude,:longitude,:email,:Foto_perfil)";
    $cad_produto = $conn->prepare($query_produto);
    $cad_produto->bindParam(':usuario', $_POST['user_url'], PDO::PARAM_STR);
    $cad_produto->bindParam(':senha',  $_POST['senha'], PDO::PARAM_STR);
    ...
    ?>
    <script type="text/javascript">   log('Cadastro Realizado Com Sucesso ');</script>
...
<form action="?acao=cadastro" method="post" enctype="multipart/form-data">
 ...
 <div class="topo_nome_login"> <label>E-mail </label></label> <input type="emil" name="email" id="enail" class="search-box_cadastro"  placeholder="E-mail"> </div> 


 <div class="topo_nome_login"> <label>Senha </label></label> <input type="password" name="senha" id="senha" class="search-box_cadastro"  placeholder="Senha"> </div> 


 <div class="topo_nome_login"> <label>Foto  </label></label> <input type="file" name="foto" id="foto" class="search-box_cadastro" required  placeholder="Foto"> </div> 
